  ...
  return (
    <Modal visible={isVisible} animationType="slide" transparent={false}>
      ...
      <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
        ...
        <View>
          <Text style={[styles.title, { color: theme.colors.text }]}>{currentTrack.filename}</Text>
          <Text style={[styles.artist, { color: theme.colors.text }]}>{currentTrack.artist}</Text>
        </View>
        ...
        <View style={styles.playBar}>
          ...
          <TouchableOpacity onPress={togglePlayback} style={[styles.playButton, styles.controlButton]}>
            <Text style={[styles.playText, { color: theme.colors.text }]}>{isPlaying ? 'Pause' : 'Play'}</Text>
          </TouchableOpacity>
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <TouchableOpacity onPress={() => playTrack(index)} style={[styles.track, index === currentTrackIndex && styles.activeTrack]}>
              <Text style={[styles.trackName, { color: theme.colors.text }]}>
                {index === currentTrackIndex ? `▶ ${item.filename}` : item.filename}
              </Text>
            </TouchableOpacity>
          )}
        />
...
const styles = StyleSheet.create({
  ...
  playBar: { flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', marginVertical: 16, paddingVertical: 8 },
  controlButton: { padding: 10 },
  controlText: { fontSize: 24 },
  playButton: { marginHorizontal: 16 },
  playText: { fontSize: 16, fontWeight: 'bold' },
  lyricsContainer: { marginVertical: 16, padding: 16, borderRadius: 8, backgroundColor: '#1a1a1a' },
