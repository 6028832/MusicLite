...
export default function Tracks() {
  ...
  return loading ? (
    ...
    <View style={styles.container}>
      <ScrollView horizontal style={styles.artistFilterContainer}>
        {Array.from(new Set(tracks.map((track) => track.artist))).map((artistName) => (
          <TouchableOpacity
            key={artistName}
            onPress={() => filterByArtist(artistName)}
            style={styles.artistFilter}
          >
            <Text style={styles.artistFilterText}>{artistName}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
